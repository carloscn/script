...
echo "CONFIG_OPTEE=y" >> ./configs/s32g2xxaevb_defconfig

make CROSS_COMPILE=aarch64-none-linux-gnu- s32g2xxaevb_defconfig
...
dd if=`pwd`/build/s32g2xxaevb/release/fip.s32 \
   of=`pwd`/build/s32g2xxaevb/release/fip_no_ivt.s32 \
   bs=1 skip=4096 seek=0 count=256 conv=notrunc && sync
__check_ret $? "gen fip_no_ivt"
